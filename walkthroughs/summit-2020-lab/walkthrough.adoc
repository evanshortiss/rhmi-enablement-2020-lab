...
= RHMI Spring Enablement 2020 Lab

Welcome to the Red Hat Spring Enablement 2020 Red Hat Managed Integration Lab!
...
+
NOTE: You can delete the existing *PostgresDB* connection on this screen using the 3 dots icon to access a menu with the *Delete* option.
...
In this section you'll deploy a pre-built implementation of the OpenAPI Specification you explored in the previous section. This API will allow you to access the Parking Meter and Junction data that your Fuse Online Integration has written to Postgres.
...
.. Select the *Image name from external registry* option.
.. For *Image Name* enter `quay.io/evanshortiss/summit-2020-rhmi-lab-nodejs-backend` and click the search icon to display the container image details in the area below the text box.
...
.. Click the *Routing* link to expand more routing options. Ensure the *Secure Route* checkbox is enabled.
.. Set *TLS Termination* to `Edge` and *Insecure Traffic* to `Redirect`.
. Lastly, scroll down and click *Create* to deploy the container image.

{blank}

At this point you've successfully created a *DeploymentConfig* for your application, but it will not start until a *Secret* containing the City Postgres connection information is created for it.

=== Create a Secret

Secrets can be used to store sensitive information securely such that it's not visible in plaintext when interacting with the OpenShift Cluster. The following steps will have you create a Secret to store the City Postgres database connection details.
...
. Choose *Workloads > Secrets* from the left-side menu and click the *Create > Key/Value Secret* button on the *Secrets* screen.
. Enter `pg-secrets` in the *Secret Name* field.
. Create a key named `username`, and enter a value of `{postgres-username}` in the text area.
...
. Verify that the {3Scale-ProductName} looks like the screenshot below.

{blank}

image::images/3scale-ui.png[3scale, role="integr8ly-img-responsive"]
...
. From the *Dashboard*, click the *New Product* link.
+
NOTE: If the *New Product* link is not visible contact the lab administrator to request access.
...
. Select the *(num) Application* item from the breadcrumb at the top of the screen beside *Account 'Developer'* to view Applications.
...
. Select the *Backends* tab from the *APIs* section.
...
[type=verification]
Are the two rules listed on the *Mapping Rules* screen?
...
.. In the *Staging Public Base URL* field enter:
+
[subs="attributes+"]
----
{3scale-staging-api-host}
----
.. Under the *Credentials Location* section choose `As query parameters`.
...
curl -H 'user_key: 7c39affada6b23445437515545ab15a4' "{3scale-staging-api-host}/meters?status=available"
----

[type=verification]
Were you able to query the API via {3Scale-ProductName} and retrieve a list of meters?
...
[time=10]
== Protect the Application using Red Hat SSO

Lastly, you will protect the frontend application from unauthorised access using Red Hat SSO.

=== Create a Realm in Red Hat SSO

Realms contain details related to a specific group of applications (*Clients*) and *Users*. A *Realm* also contains settings for *Identity Providers* and *User Ferderation* via LDAP and other protocols. You will need a realm to get started.

. Login to the link:{user-sso-url}[Customer Application SSO, window="_blank"] using your standard credentials.
. From the side menu click the *Realm* dropdown that contains the word *Master* by default.
. Select *Add Realm*:
.. Enter `{username}-realm` in the *Name* field.
.. Ensure *Enabled* is set to `ON`.
.. Click *Create*.

=== Create a Client in Red Hat SSO

A *Client* represents a specific application. In this case, you will create a client to represent the traffic application frontend. This will be a `public` client since it's used to protect a web application.

. Login to the link:{user-sso-url}[Customer Application SSO, window="_blank"] using your standard credentials.
. From the *Realm* dropdown in the side menu choose your new `{username}-realm` Realm.
. Select *Clients* on the left and click the *Create* button at the top of the list. The *Add Client* screen is displayed:
.. Enter `{is-name}` in the *Client ID* field.
.. Set *Client Protocol* to `openid-connect`.
.. Leave *Root URL* blank.
.. Click *Save*.
. The `{is-name}` client settings are displayed.
. Verify that *Access Type* is set to `public`.
. In the *Valid Redirect URIs* field enter `{route-traffic-frontend-host}*`
. Enter `{route-traffic-frontend-host}*` in the *Web Origins* field.

=== Create a User in Red Hat SSO

You need to create a user to login to the application. Generally this will be done for testing only, and a LDAP or social identity provider is used to authenticate users in production scenarios.
...
.. Click *Save*.
. Select the *Credentials* tab and enter `password` for *New Password* and *Password Confirmation*.
. Set *Temporary* to `OFF`.
. Click *Reset Password*.

=== Enable SSO for the Traffic Frontend

. Login to the link:{user-sso-url}[Customer Application SSO, window="_blank"] using your standard credentials.
. From the *Realm* dropdown in the side menu choose your new `{username}-realm` Realm.
. Select your `{is-name}` from the *Clients* list.
. Choose the *Installation* tab and change the *Format* dropdown to `Keycloak OIDC JSON`.
. Select and copy the JSON to the clipboard.
. Login to the link:{openshift-console-url}[OpenShift Console, window="_blank"].
. Select the *Developer* view using the dropdown underneath the Red Hat OpenShift logo.
. From the *Topology* view select the `{is-name}` item and select *Edit DeploymentConfig* from the *Actions* dropdown.
. Select the *Environment* tab from the *Deployment Config Details* screen.
. Create a variable named `KEYCLOAK_CONFIG` and paste the JSON you copied into the value field.
. Scroll down and click *Save*.
. Return to the *Overview* screen for the *{is-name}* and wait for the deployment to complete.

=== Test the Red Hat SSO Integration

. Open your link:{route-traffic-frontend-host}[Traffic Frontend].
. A login screen should be displayed.
. Login using the credentials `testuser` and `password`.

[type=verification]
Was the application frontend displayed after you logged in?

[type=verificationFail]
Verify that you created the `testuser` account and set the password correctly.
