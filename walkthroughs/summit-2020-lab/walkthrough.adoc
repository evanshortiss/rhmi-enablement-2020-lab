...
. Select the *Node.js* option under *Builder Image*.
...
[type=verification]
Do the logs show a successful git clone and eventually end with `Push successful`?

[type=verificationFail]
Verify that you set the *Environment Variables* for the *Build* correctly, and start the build again. Contact the administrator if the issue persists.
